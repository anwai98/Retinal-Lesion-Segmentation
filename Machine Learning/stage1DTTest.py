...
test_target = datasets.test_target
...
print("----- Binary transformed Training Data-----")
print(np.unique(train_target_bin))

test_target_bin = np.copy(test_target)
for i in range(0,test_target_bin.shape[0],1):
    if(test_target_bin[i] != 0):
       test_target_bin[i] = 1;
print("----- Binary transformed Test Data-----")
print(np.unique(test_target_bin))
...
model.fit(X_train_data, y_train_target_bin)
...
recall_1 = recall_score(test_target_bin, y_pred, pos_label=1)
print ("sensitivity for lesion: {0:3.4}%".format(recall_1*100))
recall_0 = recall_score(test_target_bin, y_pred, pos_label=0)
...
for i in range(0, y_pred.shape[0], 1):
    ...
    if(test_target_bin[i] == 1):
        among_1 = among_1 + 1
    if((test_target_bin[i] == y_pred[i]) and (test_target_bin[i] == 1)):
        found_1 = found_1 + 1
        
    if(test_target_bin[i] == 0):
        among_0 = among_0 + 1
    if((test_target_bin[i] == y_pred[i]) and (test_target_bin[i] == 0)):
        found_0 = found_0 + 1
    
    if(y_pred[i] != 0):
        if(three == 0):
            df_data = test_data[i]
            df_target = test_target[i]
            three = 1
        else:
            df_data = np.vstack((df_data,test_data[i]))
            df_target = np.vstack((df_target,test_target[i]))
...
with open('stage-1-test-green.pickle', 'wb') as f:
